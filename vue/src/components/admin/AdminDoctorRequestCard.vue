  ...
  <div class="justify-content-center" style="margin: auto; max-width: 1500px">
    <b-card class="profile" style="width: 1400px;">
      <div class="row">
        ...
        <div class="column" style="margin: auto; width: 150px;">
          <b-card-text style="margin-bottom: 10px; color: #355454">{{ doctor_data.license }}</b-card-text>
        </div>
        <div class="column" style="margin-left: auto; margin-right: 5px">
          <b-button variant="info" v-on:click="move_to_accept">{{ $t('accept') }}</b-button>
        </div>
        <div class="column" style="margin-left: auto;">
          <b-button variant="info" v-on:click="move_to_delete">{{ $t('reject-request') }}</b-button>
        </div>
      </div>
    </b-card>
  </div>
...
export default {
  ...
  methods: {
    move_to_accept() {
      axios
          .patch( 'http://localhost:5000/doctorrequest/' + this.doctor_data._id)
          .then(response => {
            if (response.data) {
              window.location.reload();
              this.$toast.success(this.$t('accept-success'))
            }
          })
          .catch(err => {
            if (err.response) {
    ...
    move_to_delete() {
      axios
          .delete( 'http://localhost:5000/doctorrequest/' + this.doctor_data._id)
          .then(response => {
            if (response.data) {
              window.location.reload();
